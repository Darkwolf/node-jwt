...
import {
  encodeText as Base64URLEncodeText,
  decodeText as Base64URLDecodeText,
  encodeToString as Base64URLEncodeToString,
  decodeFromString as Base64URLDecodeFromString
} from '@darkwolf/base64url'
...
const _encodeUnsecured = (header, payload) => {
  ...
  return `${encodedHeader}${SEPARATOR_CHAR}${encodedPayload}`
}
...
const _encode = (header, payload, signature) => {
  ...
  if (algorithm !== NONE) {
    ...
    result += `${SEPARATOR_CHAR}${encodedSignature}`
  }
  return result
}
...
const _decodeUnsecured = (token, encoded) => {
  const [encodedHeader, encodedPayload] = StringPrototypeSplit(token, SEPARATOR_CHAR, 2)
  if (!encodedHeader) {
...
const sign = (header, payload, key) => {
  ...
  let result = `${encodedUnsecured}${SEPARATOR_CHAR}`
  if (algorithm !== NONE) {
...
const verify = (token, key, options) => {
  ...
  if (algorithm !== NONE) {
    const encodedUnsecured = `${encodedHeader}${SEPARATOR_CHAR}${encodedPayload}`
...
class JWT {
  ...
  [verifySignatureSymbol](key) {
    ...
    const encodedUnsecured = `${this[encodedHeaderSymbol]}${SEPARATOR_CHAR}${this[encodedPayloadSymbol]}`
